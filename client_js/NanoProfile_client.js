...
const nanoProfile = (function () {
  ...
  function registerImage (imageHash) {
    ...
    return {destination: CONFIG.trackerAccount, amount: CONFIG.imageCode, representative: representative}
  }
  ...
  function importBlacklist(accounts) {
    for (let i in accounts){
      if (checkNanoAddress(accounts[i])) {
        BLACKLIST.push(accounts[i])
      } else {
        throw new Error ("Invalid account: " + accounts[i])
      }
    }
  }
      ...
      for (let blockN in register_transactions) {
        ...
        if ( ! (block in registered_blocks) ) {
          let blockInfoPromise = block_info(block).then ( function (transaction) {
            ...
            accountsImagesRegisters[account].push({block: block, imageHash: imageHash})
            if ( !(account in newAccountsImagesRegisters) ) newAccountsImagesRegisters[account] = []
            newAccountsImagesRegisters[account].push({block: block, imageHash: imageHash})
            if (callback && !BLACKLIST.includes(account)) callback({foundRegister: block, account: account, imageHash: imageHash})
          })
      ...
      await Promise.all(blockInfoAllPromises).then( function () {
        for (let account in newAccountsImagesRegisters){
          ...
          if (newRegisters == 1) {
            ...
            if (callback && !BLACKLIST.includes(account)) callback({currentRegister: accountsImages[account].block, account: account, imageHash: accountsImages[account].imageHash, imageLink: CONFIG.ipfsGateway + accountsImages[account].imageHash})
          }
          if (newRegisters > 1){
  ...
  async function determineLastImageRegister (account){
    ...
    for (let transactionN in history){
      ...
      if (transaction.type == "send" && transaction.account == CONFIG.trackerAccount && transaction.amount == CONFIG.imageCode){
        ...
        return {block: blockInfo.hash, imageHash: imageHash}
      }
    }
    return undefined
  }

  async function pending_filter(account, amount, count) {
    let validTransactions = []
    try {
      const request = await pending_transactions(account, amount)
      if ("error" in request) return {error: request.error}
      const blocks = request.blocks
      for (let block in blocks){
        if (blocks[block] == amount) validTransactions.push(block)
      }
